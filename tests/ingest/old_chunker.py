...
class MarkdownChunker:
    ...
    def process_text(self, text, source_name="text_input"):
        """Xử lý text trực tiếp thay vì từ file."""
        logger.info(f"Starting chunking process for text input: {source_name}")
        ...
        logger.info(f"Extracted {len(headers)} headers from {source_name}")
    ...
    def process_directory(self, input_dir):
        ...
        for file_path in md_files:
            ...
            try:
                results = self.process_and_merge(file_path)
                all_results.extend(results)  # Add results from this file to the combined list
                processed_count += 1
                logger.info(f"Created {len(results)} chunks for {filename}")
                
            except Exception as e:
                logger.error(f"Error processing {filename}: {str(e)}")
        
        logger.info(f"Completed processing {processed_count} files, total chunks: {len(all_results)}")
        return all_results
...
if __name__ == '__main__':
    ...
    all_results = chunker.process_directory(input_dir)
    
    logger.info(f"Total chunks created from all files: {len(all_results)}")
